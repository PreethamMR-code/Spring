...
<head>
    ...
    <style>
        :root { --sidebar-bg:#1e293b; --sidebar-hover:#334155; --accent-color:#38bdf8; }
        body { overflow-x:hidden; background-color:#f8fafc; }
        .sidebar { position:fixed; top:0; left:-280px; width:280px; height:100vh; background:var(--sidebar-bg); transition:all 0.4s cubic-bezier(0.4,0,0.2,1); z-index:1060; box-shadow:4px 0 25px rgba(0,0,0,0.15); overflow-y:auto; border-right:1px solid rgba(255,255,255,0.05); }
        ...
        .sidebar-menu .menu-item { padding:12px 18px; color:#94a3b8; text-decoration:none; display:flex; align-items:center; border-radius:8px; margin-bottom:4px; font-weight:500; font-size:0.95rem; transition:all 0.2s ease; }
        ...
        .menu-toggle { position:fixed; top:15px; left:15px; width:45px; height:45px; background:white; border:1px solid #e2e8f0; border-radius:12px; cursor:pointer; display:flex; flex-direction:column; justify-content:center; align-items:center; gap:4px; z-index:1055; box-shadow:0 4px 6px -1px rgba(0,0,0,0.1); transition:all 0.3s ease; }
        ...
        .sidebar-overlay { position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(15,23,42,0.6); backdrop-filter:blur(4px); z-index:1050; display:none; opacity:0; transition:opacity 0.3s ease; }
...
<%-- Resolve name and email: model takes priority, session is fallback --%>
...
<div class="sidebar" id="sidebar">
    ...
    <div class="sidebar-menu">
        ...
        <%-- ✅ FIX 3: Use absolute URL with c:url + session fallback for email --%>
        <a href="<c:url value='/editProfile'/>?email=${displayEmail}" class="menu-item">
        ...
        <%-- ✅ FIX 3: href="/logout" controller — NOT relative "signIn" --%>
                    ...
                    <ul class="navbar-nav ms-auto align-items-center">
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle d-flex align-items-center" href="#" data-bs-toggle="dropdown">
                                <%-- ✅ FIX 2: Each user sees THEIR OWN photo via their own fileId in session --%>
                                <c:choose>
                                    <c:when test="${not empty displayFileId}">
                                        <img src="<c:url value='/getImage?id=${displayFileId}'/>"
                                ...
                                <span class="fw-semibold">${not empty displayName ? displayName : 'User'}</span>
                            </a>
                            ...
                            <ul class="dropdown-menu dropdown-menu-end shadow" style="min-width:300px;">
                                ...
                                <li>
                                    <%-- ✅ FIX 3: Absolute URL --%>
                                    <a class="dropdown-item" href="<c:url value='/editProfile'/>?email=${displayEmail}">
                                        <i class="bi bi-pencil-square me-2"></i>Edit Profile
                                    </a>
                                </li>
                                <li><hr class="dropdown-divider"></li>
                                <li>
                                    <%-- ✅ FIX 3: /logout controller, NOT relative "signIn" --%>
            ...
            <div class="container">
                <div class="row align-items-center">
                    <div class="col-lg-8 text-center text-md-start">
                        <h1 class="display-4 fw-bold mb-3">Welcome Back, ${displayName}!</h1>
...
<!-- UPLOAD PHOTO MODAL — must be outside navbar -->
            ...
            <form action="<c:url value='/uploadProfilePhoto'/>" method="post" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="text-center mb-3">
                        <img id="photoPreview"
                             src="<c:choose>
                                    <c:when test='${not empty displayFileId}'><c:url value='/getImage?id=${displayFileId}'/></c:when>
                                    <c:otherwise>https://ui-avatars.com/api/?name=${displayName}&size=150&background=0d6efd&color=fff</c:otherwise>
                                  </c:choose>"
                             class="rounded-circle mb-2" width="150" height="150"
                             style="object-fit:cover; border:3px solid #dee2e6;" alt="Preview">
